...
// If true, we have our ability and its not locked. If false, ability is locked and text written below is BLOCKED
	...
	FString& OutNextLevelDescription)
{
	...
	const UAbilityInfo* const AbilityInfo = UAuraAbilitySystemBPLibrary::GetAbilityInfo(GetAvatarActor());
	...
	if(FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		if(bStatusValid)
		{
			...
			if (!SlotIsEmpty(Slot)) // There is ability in this slot already. Deactivate and clear its slot.
			{
				...
				if (SpecWithSlot)
				{
					// Is that ability the same as thisa ability? If so, we can return early.
...
void UAuraAbilitySystemComponent::UpdateAbilityStatuses(int32 Level)
{
	...
	for(const FAuraAbilityInfo& info : AbilityInfo->AbilityInformation)
	{
		...
		if(GetSpecFromAbilityTag(info.AbilityTag) == nullptr) //character don`t have ability
		{
...
void UAuraAbilitySystemComponent::ServerSpendSpellPoint_Implementation(const FGameplayTag& AbilityTag)
{
	if(FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		{
			// Leveling up ability without canceling
			AbilitySpec->Level += 1;
		}
