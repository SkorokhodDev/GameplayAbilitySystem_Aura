...
UAuraAttributeSet::UAuraAttributeSet()
{
	...
	// and have a var TMap<FGameplayTag, FAttributeSignature> TagsToAttributes;
...
void UAuraAttributeSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	// Registering health to be replicated
...
void UAuraAttributeSet::HandleIncomingXP(const FEffectProperties& Props)
{
	...
	if (Props.SourceCharacter->Implements<UPlayerInterface>() && Props.SourceCharacter->Implements<UCombatInterface>())
	{
		...
		// Finding how much levels we can level up
		const int32 NewLevel = IPlayerInterface::Execute_FindLevelForXP(Props.SourceCharacter, CurrentXP + localIncomingXP);
		const int32 NumberLevelsUps = NewLevel - CurrentLevel;

		if(NumberLevelsUps > 0)
		{
			//TODO: GetAttributePointsReward and SpellPoints Reward
			...
			IPlayerInterface::Execute_AddToPlayerLevel(Props.SourceCharacter, NumberLevelsUps);
			IPlayerInterface::Execute_AddToAttributePoints(Props.SourceCharacter, AttributePointsReward);
			IPlayerInterface::Execute_AddToSpellPoints(Props.SourceCharacter, SpellPointsReward);

			// FIll our Health and Mana
			bTopOffHealth = true;
			bTopOffMana = true;
...
void UAuraAttributeSet::Debuff(const FEffectProperties& Props)
{
	...
	// HERE WE APPLY OUR DebuffTag to target actor

	FGameplayTag DebuffTag = GameplayTags.DamageTypesToDebuffs[DamageType];
	
	//Effect->InheritableOwnedTagsContainer.AddTag(DebuffTag); // DEPRECEATED
	...
	// We can create levels for debuffs and use scalable float 
...
void UAuraAttributeSet::SetEffectProperties(const FGameplayEffectModCallbackData& Data, FEffectProperties& Props) const
{
	...
	/* Source = causer of the effect, Target = target of the effect (if our case owner of this AttribSet) */
	...
	if (IsValid(Props.SourceASC) && Props.SourceASC->AbilityActorInfo.IsValid() && Props.SourceASC->AbilityActorInfo->AvatarActor.IsValid())
	{
		...
		{
			if (const APawn* const Pawn = Cast<APawn>(Props.SourceAvatarActor))
			{
...
void UAuraAttributeSet::ShowFloatingText(const FEffectProperties& Props, float Damage, bool bBlockedHit, bool bCriticalHit) const
{
	...
	{
		// Damage causer is player controlled Character
